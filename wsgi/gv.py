...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_gv = tempfile.NamedTemporaryFile(suffix=".gv", delete=False)
            temp_gv_name = temp_gv.name
            temp_gv.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_gv_name)

            with open(temp_gv_name, "rb") as gv_file:
                gv_data = gv_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'text/vnd.graphviz'),
                ('Content-Length', str(len(gv_data))),
            ])
            return [gv_data]
