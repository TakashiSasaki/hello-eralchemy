...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_json0 = tempfile.NamedTemporaryFile(suffix=".json0", delete=False)
            temp_json0_name = temp_json0.name
            temp_json0.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_json0_name)

            with open(temp_json0_name, "rb") as json0_file:
                json0_data = json0_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'application/json'),
                ('Content-Length', str(len(json0_data))),
            ])
            return [json0_data]
