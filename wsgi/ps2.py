...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_ps2 = tempfile.NamedTemporaryFile(suffix=".ps2", delete=False)
            temp_ps2_name = temp_ps2.name
            temp_ps2.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_ps2_name)

            with open(temp_ps2_name, "rb") as ps2_file:
                ps2_data = ps2_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'application/postscript'),
                ('Content-Length', str(len(ps2_data))),
            ])
            return [ps2_data]
