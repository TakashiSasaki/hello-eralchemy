...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_xdot_json = tempfile.NamedTemporaryFile(suffix=".xdot_json", delete=False)
            temp_xdot_json_name = temp_xdot_json.name
            temp_xdot_json.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_xdot_json_name)

            with open(temp_xdot_json_name, "rb") as xdot_json_file:
                xdot_json_data = xdot_json_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'application/json'),
                ('Content-Length', str(len(xdot_json_data))),
            ])
            return [xdot_json_data]
