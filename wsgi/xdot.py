...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_xdot = tempfile.NamedTemporaryFile(suffix=".xdot", delete=False)
            temp_xdot_name = temp_xdot.name
            temp_xdot.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_xdot_name)

            with open(temp_xdot_name, "rb") as xdot_file:
                xdot_data = xdot_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'application/vnd.graphviz'),
                ('Content-Length', str(len(xdot_data))),
            ])
            return [xdot_data]
