...
def application(environ, start_response):
    ...
    if method == 'POST':
        try:
            ...
            temp_ps = tempfile.NamedTemporaryFile(suffix=".ps", delete=False)
            temp_ps_name = temp_ps.name
            temp_ps.close()
            ...
            render_er(f"sqlite:///{temp_db_name}", temp_ps_name)

            with open(temp_ps_name, "rb") as ps_file:
                ps_data = ps_file.read()

            start_response('200 OK', [
                ('Access-Control-Allow-Origin', '*'),
                ('Content-Type', 'application/postscript'),
                ('Content-Length', str(len(ps_data))),
            ])
            return [ps_data]
